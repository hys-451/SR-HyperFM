...
def gat_attention(queries,
    ...
    # Linear projections
    with tf.variable_scope("att_blocks%d" % block):
    ...
    with tf.variable_scope("w_blocks%d" % block):
        # if field > 0: tf.get_variable_scope().reuse_variables()
    ...
    if has_residual:
        with tf.variable_scope("res_blocks%d" % block):
            # if field > 0: tf.get_variable_scope().reuse_variables()
            Q_res = tf.layers.dense(queries, num_units, activation=tf.nn.relu,
    ...
    with tf.variable_scope("gsl_blocks%d" % block):
        if field > 0: tf.get_variable_scope().reuse_variables()
        S = tf.layers.dense(values, 16, activation=tf.nn.relu, name='gsl_1_b%d' % (block))
        ...
        kth = tf.reduce_min(values, -1, keep_dims=True)
    ...
    # Activation
    weights = tf.nn.softmax(A_, dim=1)
    ...
    outputs = tf.reduce_sum(outputs, axis=1, keep_dims=True)  # [num_heads*batch_size, 1, num_units/num_heads]
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            for i in range(self.blocks):
                state_list = []#对于每一个block，初始化一个空列表，用于存储每个field的状态
                vis_list = []
                for j in range(self.field_size):
                    state = tf.expand_dims(h[:, j, :], 1)  # [batch_size, 1, hidden_size]#从h中取出第j个field的嵌入，然后通过tf.expand_dims增加一个维度，使其从[batch_size, embedding_dimension]变为[batch_size, 1, embedding_dimension]。
                    state_all = state * h  # [batch_size, field_size, hidden_size]#对于第j个field，计算它与所有其他fields的嵌入的逐元素乘积
